...
class semiGAN_MultiLabelMAPEngine(MultiLabelMAPEngine):
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True, semi_supervised=False):
        ...
        feature_error = torch.mean(torch.mean(torch.mean(features.detach(), dim=0) - torch.mean(x_g[:,:features.shape[1],:], dim=0), dim=0))
        ...
        # print(G_feat_match)
        g_loss = G_feat_match#g_loss #+G_feat_match#
